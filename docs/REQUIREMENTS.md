...
- **REQ-CORE-008 (Geräte-Bündelung / Subentry-Clustering):** Die Integration MUSS es erlauben, mehrere Abstract-Sensoren als Subentries unter einem einzigen Root-Config-Entry zu organisieren; jede Subentry behält dabei eine unabhängig stabile Identität (siehe REQ-CORE-001), unabhängig von einer späteren Änderung ihrer Geräte-Zuordnung. Das Bündeln mehrerer Subentries auf ein gemeinsames HA-Gerät über die UI ist derzeit zurückgestellt: Das aktuelle Geräte-Registry-Verhalten von Home Assistant erlaubt es nicht, die Geräte-Zuordnung einer Subentry zu ändern, ohne den Entity-Registry-Eintrag einer anderen Subentry auf demselben Gerät zu zerstören ([GH#18](https://github.com/Popoboxxo/AbstrHActor/issues/18)). Der zugrunde liegende `device_group_id`-Mechanismus (`sensor.py`, `config_flow.py::_normalize`) existiert und ist unit-getestet, ist über die UI aber nicht mehr erreichbar (die Formularfelder `target_device_id`/`create_new_device` wurden aus `_schema()` entfernt) und wird auch vom Legacy-Migrationspfad nicht gesetzt — dieser bündelt Sensoren stattdessen implizit, indem er die alte Config-Entry-ID als `subentry_id` weiterverwendet, ohne `device_group_id` zu berühren. **Status:** Teilweise implementiert — Fundament in 1.1.0-rc.1 ausgeliefert, Bündelungs-UI zurückgestellt.
